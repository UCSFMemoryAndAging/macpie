...
def test_dfs(tmp_path):
    reg_df.to_excel(tmp_path / "reg_df.xlsx")
    reg_df_parsed = pd.read_excel(tmp_path / "reg_df.xlsx", index_col=0)
    assert_frame_equal(reg_df_parsed, reg_df)

    mi_df.to_excel(tmp_path / "mi_df.xlsx")
    mi_df_parsed = pd.read_excel(tmp_path / "mi_df.xlsx", index_col=[0, 1], header=[0, 1])
    assert_frame_equal(mi_df_parsed, mi_df)


def test_1_1_permutations(tmp_path):
    df_1_1 = pd.DataFrame(data, columns=reg_columns, index=reg_index)

    dset_1_1 = mp.Dataset(df_1_1)

    dset_1_1.to_excel(tmp_path / "dset_1_0.xlsx", header=True, index=False)
    dset_1_1.to_excel(tmp_path / "dset_0_1.xlsx", header=False, index=True)
    dset_1_1.to_excel(tmp_path / "dset_1_1.xlsx", header=True, index=True)
    dset_1_1.to_excel(tmp_path / "dset_0_0.xlsx", header=False, index=False)
...
def test_1_2(tmp_path):
    ...
    dset_1_2.to_excel(tmp_path / "dset_1_2.xlsx", header=True, index=True)
...
def test_basic_collection(tmp_path):
