...
def test_create_output_dir(tmp_path):
    ...
    with pytest.raises(errors.PathError):
        io.create_output_dir(Path('tests/io/data/test.csv').resolve())
...
def test_get_files_from_dir():
    files = io.get_files_from_dir(current_dir)
...
def test_validate_filepaths():
    ...
    (valid, invalid) = io.validate_filepaths([current_dir], allowed_file)
